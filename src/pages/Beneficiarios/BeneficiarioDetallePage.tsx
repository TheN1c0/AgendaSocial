...
const CASOS_ASOCIADOS_MOCK = [
  { id: '#4512', estado: 'en_proceso', prioridad: 'alta',  profesional: 'Marta Gómez', fechaIngreso: '12/10/2023' },
  { id: '#4510', estado: 'cerrado',    prioridad: 'baja',  profesional: 'Diego Rivas', fechaIngreso: '05/10/2023' },
  { id: '#4507', estado: 'cerrado',    prioridad: 'media', profesional: 'Marta Gómez', fechaIngreso: '01/08/2023' },
];
...
export const BeneficiarioDetallePage = () => {
  ...
  const bMock = BENEFICIARIOS_MOCK.find(b => b.id === id) || BENEFICIARIOS_MOCK[0];
  const [beneficiario, setBeneficiario] = useState(bMock);
           ...
           <div className="flex flex-col gap-1 w-full md:w-auto">
             ...
             <div className="flex flex-col sm:flex-row gap-2 sm:gap-6 text-sm text-gray-600 dark:text-gray-400 mt-1">
               ...
               <span className="flex items-center gap-1">
                 ...
                 <span className="font-medium text-gray-800 dark:text-gray-200">{beneficiario.profesionalAsignado}</span>
               </span>
             </div>
           </div>
        </div>
          ...
          <Card title="Datos de contacto y personales">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-6 text-sm">
              ...
              <div className="flex flex-col border-b border-gray-100 dark:border-gray-800 pb-2">
                ...
                <span className="font-medium text-gray-900 dark:text-gray-100 flex items-center gap-2">
                  <span>🎂</span>{beneficiario.fechaNacimiento} <span className="text-gray-500 font-normal">({edad} años)</span>
                </span>
              </div>
            </div>
            ...
            <div className="border border-gray-100 dark:border-gray-800 rounded-xl overflow-hidden overflow-x-auto mt-2">
              <table className="w-full text-left text-sm whitespace-nowrap">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {CASOS_ASOCIADOS_MOCK.map(c => (
                    <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-[#2a2a2a] transition-colors cursor-pointer group" onClick={() => navigate(`/casos/${c.id.replace('#','')}`)}>
                      <td className="px-4 py-3 font-medium text-primary group-hover:underline">
                        {c.id}
                      </td>
                      <td className="px-4 py-3">
                        <Badge estado={c.estado as EstadoCaso}>{c.estado.replace('_', ' ').replace(/\b\w/g, char => char.toUpperCase())}</Badge>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <Avatar name={c.profesional} size="sm" className="w-[20px] h-[20px] text-[9px]" />
                          <span className="text-gray-700 dark:text-gray-300">{c.profesional}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-gray-500">{c.fechaIngreso}</td>
                    </tr>
                  ))}
                  {CASOS_ASOCIADOS_MOCK.length === 0 && (
                    <tr>
          ...
          <Card title="Resumen">
            <div className="flex flex-col gap-4">
              ...
              <div className="flex justify-between items-center bg-primary/5 border border-primary/20 p-3 rounded-xl">
                ...
                <span className="text-2xl font-black text-primary">{beneficiario.casosActivos}</span>
              </div>
              ...
              <div className="flex justify-between items-center bg-gray-50 dark:bg-[#242424] border border-gray-100 dark:border-gray-800 p-3 rounded-xl">
                ...
                <span className="text-xl font-bold text-gray-800 dark:text-gray-200">{beneficiario.casosTotales}</span>
              </div>
              ...
              <div className="flex justify-between items-center text-sm border-b border-dashed border-gray-200 dark:border-gray-800 pb-2 mt-2">
                ...
                <span className="font-medium text-gray-900 dark:text-gray-100">{beneficiario.ultimaActividad}</span>
              </div>
              ...
              <div className="flex flex-col gap-2 mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                ...
                <Button variant="secondary" onClick={() => navigate(`/casos?beneficiarioId=${beneficiario.id}`)} className="w-full">
                  Ver todos sus casos →
                </Button>
              </div>
            </div>
      ...
      <Modal isOpen={modalEditar} onClose={() => setModalEditar(false)} title="Editar datos del beneficiario">
        <form onSubmit={handleEditSave} className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Input name="fechaNacimiento" defaultValue={beneficiario.fechaNacimiento === 'Sin registrar' ? '' : beneficiario.fechaNacimiento} placeholder="DD/MM/YYYY" label="Fecha de Nacimiento" />
          </div>
